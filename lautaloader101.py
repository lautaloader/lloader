...
# TODO: we are ignoring naamapalmu. not sure if want
...
from tkinter import (Button, Checkbutton, DISABLED, E, Entry, END, FALSE,
                     ...
                     Radiobutton, StringVar, SUNKEN, Tk, W)
...
class Main:
    def __init__(self, master):  # we will define everything in the UI below
        self.master = master
        self.master.wm_title("Lautaloader v.1.01")  # title of window
    ...
    def logic(self):
        ...
        while not done and self.check_for_url() is True:
            ...
            for imglink in soup.select(".filecontainer figcaption a"):  # loop through
                total_stuff += 1  # we make this to help with progressbar

            for imglink in soup.select(".filecontainer figcaption a"):  # loop again and process
                try:
                ...
                self.progressbar['value'] += 100 / total_stuff  # after file process we add to progressbar
